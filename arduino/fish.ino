...
// 수온: 2, LED: 3, 모터: 5,  RTC: 6~8, SD: 4, WiFi: 10~13

const int PIN_TEMP = 2;
const int PIN_LEDS = 3;
const int PIN_SD = 4;
const int PIN_MOTOR = 5;
const uint8_t PIN_RST = 6;
...
String nowTime, pastTime; // 1초 단위로 시간을 파악하기 위해 이전 시간과 현재 시간을 비교할 변수들.
...
void setup() {
  ...
  // 시리얼 통신을 하지 않는 경우에 LED가 깜빡임으로써 준비가 완료됐다는 걸 알리기 위함.
...
void loop() {
  ...
  // 시간을 받아옴.
  nowTime = getTime();
  // 이전 시간은 초기에 한 번 설정해줘야 하고, 이전 시간과 현재 시간이 다를 경우에만
  if(nowTime != pastTime || pastTime == NULL) {
    pastTime = nowTime;
    ...
    } else if(offTime.indexOf(nowTime) == 0) { // 현재 시간과 자동 조명 OFF 시간이 일치하면
